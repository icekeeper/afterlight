...
[Download for Windows 11](https://github.com/icekeeper/afterlight/releases/latest) · [Watch the complete demo](https://github.com/icekeeper/afterlight/releases/download/v1.4.0/AFTERLIGHT-visual-preview.mp4)
...
The build uses the portable **Zig 0.13.0** C++ toolchain. Visual Studio and the Windows SDK do not need to be installed. From PowerShell in the source directory:

```powershell
powershell -ExecutionPolicy Bypass -File .\build.ps1 -DownloadToolchain
```

The first build downloads Zig from its [official download site](https://ziglang.org/download/), verifies the archive against the pinned SHA-256 published in the [official release index](https://ziglang.org/download/index.json), and extracts it into `tools`. Later builds reuse it. All compiler caches stay inside `build`; nothing is installed system-wide. If Zig is already available elsewhere:
